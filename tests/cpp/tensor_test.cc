...
TEST(TensorTest, TensorIdDistinctAcrossCopyConstruction) {
  ...
  Tensor b(a);
  EXPECT_NE(a.tensor_id(), b.tensor_id());
  // Copying is a deep, independent copy: mutating one's content afterward
  // (as several onnxoptimizer passes do -- fetch-by-copy, mutate the local
  // copy, add it to the graph as new content) must not affect the other's.
  b.sizes().push_back(4);
...
TEST(TensorTest, TensorIdRefreshedByCopyAssignment) {
  ...
  b = a;
  // b's content just changed (even if, as here, to something that happens to
  // look the same) -- its id must move on from whatever it was before, so a
  // cache entry keyed on the old id is never silently reattached to it.
