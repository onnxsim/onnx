  ...
  bool ProcessNode(
    ...
    ONNX_TRY {
      ...
      for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        // mergeShapesAndTypes (unlike this function) returns void and always
        // mutates `existing` in place -- matching onnx's own protobuf-based
        // UpdateType, which does the same to its stored TypeProto
        // unconditionally, with no "did this change anything" signal of its
        // own. This loop needs that signal to report back to Run()'s caller
        // (the outer fixed point that decides whether to keep iterating), so
        // detect it here by comparing the serialized form before and after.
